...
const TARGET_WORKOUT = {
  ...
  sections: [
    { label: "Mile 1", detail: "Target 11:00", icon: "spear" },
    { label: "5 Pull-ups", detail: "Banded · ~2 min", icon: "tower" },
    { label: "75 Push-ups", detail: "Full · ~15 min", icon: "shield" },
    { label: "300 Squats", detail: "Air · ~12 min", icon: "anvil" },
    { label: "Mile 2", detail: "Target 13:00", icon: "sword" },
  ],
...
OPTIONAL_SESSIONS.forEach(s => {
  DAY_INDEX[s.id] = { def: { id: s.id, day: s.name, hasTime: false, exercises: [] }, group: "Optional session", accent: "#5f4c33" };
});
DAY_INDEX[TRIAL.id] = {
...
LEGACY_DAYS.forEach(d => {
  DAY_INDEX[d.id] = { def: d, group: "Old program · Build the Base", accent: "#6b5639", legacy: true };
});
...
const DISPLAY_SC = "'IM Fell English SC', Georgia, serif";

// Engraved-style line icons, drawn in the current text color.
const ICON_PATHS = {
  spear: <><path d="M12 9v13M10 12h4" /><path d="M12 2l3 6-3 2-3-2z" fill="currentColor" /></>,
  tower: <><path d="M5 21V6h2.5v2H10V6h4v2h2.5V6H19v15z" /><path d="M10 21v-4a2 2 0 0 1 4 0v4" /></>,
  shield: <><path d="M12 3l7 2.5V11c0 5-3.5 8.5-7 10-3.5-1.5-7-5-7-10V5.5z" /><path d="M12 3v18M5.5 11h13" /></>,
  anvil: <><path d="M3 7h13c0 2.5 2 4 5 4v1h-5c-1 0-2 1-2 2v2h2v2H8v-2h2v-2c0-1.5-1.5-2.5-3-2.5-2 0-4-1.5-4-4.5z" /></>,
  sword: <><path d="M12 2l1.5 3v10h-3V5z" /><path d="M7 15h10M12 15v4" /><circle cx="12" cy="20.5" r="1.5" /></>,
};

function Icon({ name, size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"
      strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ display: "inline-block", verticalAlign: "-0.2em" }}>
      {ICON_PATHS[name]}
    </svg>
  );
}
...
const inputStyle = (accent) => ({
  background: "#e9dcc0", border: `1px solid ${accent}40`, borderRadius: 5,
  color: "#3b2a1a", fontSize: 13, padding: "5px 9px", fontFamily: "inherit",
  outline: "none", width: "100%", boxSizing: "border-box",
});

const labelStyle = {
  fontSize: 11, color: "#6b5639", letterSpacing: "0.08em",
...
const checkboxStyle = (checked, accent, size) => ({
  ...
  border: `2px solid ${checked ? accent : "#a89070"}`,
...
function Checkbox({ checked, accent, size = 20, onClick }) {
  return (
    <button onClick={onClick} style={checkboxStyle(checked, accent, size)}>
      {checked && <span style={{ color: "#e9dcc0", fontSize: size > 20 ? 13 : 11, fontWeight: 900 }}>✓</span>}
    </button>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        ...
        <span style={{ fontSize: 13, color: stat.done ? accent : "#3b2a1a", fontWeight: 600, flex: 1 }}>{ex.name}</span>
        {ex.sets && (
      ...
      {ex.note && <div style={{ fontSize: 11, color: "#6b5639", marginTop: 4, marginLeft: checkable ? 28 : 0, lineHeight: 1.5 }}>{ex.note}</div>}
      {hasInputs && (
...
export default function MurphPlan() {
  ...
  const syncBadge = (() => {
    if (!loaded) return { text: "Loading…", color: "#5f4c33" };
    if (syncStatus === "saving") return { text: "Saving…", color: "#5f4c33" };
    if (syncStatus === "error") return { text: "Sync error", color: "#9b2c2c" };
    return { text: "Synced", color: "#556b2f" };
  })();
  ...
  const sectionHeading = (accent) => ({
    fontFamily: DISPLAY_SC, fontSize: 15, letterSpacing: "0.08em", color: accent, marginBottom: 12,
  });

  // ── render ────────────────────────────────────────────────────────────────

  return (
    <div style={{ background: "#e9dcc0", minHeight: "100vh", fontFamily: "'Georgia', serif", color: "#3b2a1a", paddingBottom: 60 }}>

      {/* ── Header ── */}
      <div style={{ background: "linear-gradient(180deg, #f7efdc 0%, #e9dcc0 100%)", borderBottom: "1px solid #cdb893", padding: "28px 24px 20px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ fontFamily: DISPLAY_SC, fontSize: 13, letterSpacing: "0.2em", color: "#7a4e2d" }}>12-Week Ascension</div>
            <div style={{ fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: syncBadge.color }}>{syncBadge.text}</div>
          </div>
          <h1 style={{ fontFamily: DISPLAY, fontSize: 36, fontWeight: 400, margin: "0 0 4px", color: "#2a1c10" }}>The Umbral Order</h1>
          <p style={{ margin: "0 0 20px", color: "#5f4c33", fontSize: 14 }}>Rise through the ranks. Advance when you're ready.</p>
          <div style={{ background: "#f7efdc", border: "1px solid #cdb893", borderRadius: 10, padding: "14px 18px" }}>
            <div style={{ fontFamily: DISPLAY_SC, fontSize: 14, letterSpacing: "0.12em", color: "#7a4e2d", marginBottom: 12 }}>{TARGET_WORKOUT.subtitle}</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
              {TARGET_WORKOUT.sections.map((s, i) => (
                <div key={i} style={{ background: "#efe3c8", border: "1px solid #cdb893", borderRadius: 6, padding: "8px 10px", flex: "1 1 auto", minWidth: 90 }}>
                  <div style={{ color: "#7a4e2d", marginBottom: 4 }}><Icon name={s.icon} size={30} /></div>
                  <div style={{ fontWeight: 700, color: "#2a1c10", fontSize: 12 }}>{s.label}</div>
                  <div style={{ color: "#6b5639", fontSize: 10, marginTop: 1 }}>{s.detail}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize: 11, color: "#7a4e2d", borderTop: "1px solid #cdb893", paddingTop: 10 }}>→ {TARGET_WORKOUT.finalForm}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 24px" }}>
        ...
        {/* ── Tabs ── */}
        <div style={{ display: "flex", borderBottom: "1px solid #cdb893", margin: "20px 0 0" }}>
          {["plan", "log"].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              background: "none", border: "none",
              borderBottom: activeTab === tab ? `2px solid ${phase.accent}` : "2px solid transparent",
              color: activeTab === tab ? "#2a1c10" : "#75603f",
              padding: "8px 18px 10px", fontSize: 17, fontWeight: 400,
              cursor: "pointer", letterSpacing: "0.06em",
              fontFamily: DISPLAY_SC, marginBottom: -1,
            }}>
          ...
          <>
              ...
              {phases.map((p, i) => {
                ...
                return (
                  <button key={i} onClick={() => { setActivePhase(i); setOpenDay(null); }} style={{
                    background: activePhase === i ? p.accent : "#f7efdc",
                    color: activePhase === i ? "#e9dcc0" : done ? p.accent : "#5f4c33",
                    border: `1px solid ${activePhase === i ? p.accent : done ? p.accent + "60" : "#cdb893"}`,
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 3 }}>
                <span style={{ fontFamily: DISPLAY, fontSize: 36, color: phase.accent }}>{phase.phase}</span>
                <span style={{ fontFamily: DISPLAY, fontSize: 24, color: "#2a1c10" }}>{phase.name}</span>
              </div>
              <p style={{ margin: "0 0 6px", color: "#5f4c33", fontSize: 13 }}>{phase.tagline}</p>
              ...
              <div style={{ display: "flex", gap: 12, fontSize: 12, color: "#5f4c33", marginBottom: 6 }}>
                <span>{completedMilestones}/{phase.milestones.length} milestones</span>
              </div>
              <div style={{ height: 4, background: "#cdb893", borderRadius: 2, overflow: "hidden" }}>
            ...
            {/* Pull-up callout */}
            <div style={{ background: "#f7efdc", borderLeft: `3px solid ${phase.accent}`, borderRadius: "0 6px 6px 0", padding: "9px 12px", marginBottom: 18, fontSize: 13, color: "#4a3826" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              {phase.schedule.map((day, di) => {
                ...
                return (
                  <div key={di} style={{ marginBottom: 7 }}>
                    <div style={{
                      background: isOpen ? "#efe3c8" : "#f7efdc",
                      border: `1px solid ${isOpen ? phase.accent + "50" : checked ? phase.accent + "40" : "#cdb893"}`,
                      ...
                      <button onClick={() => setOpenDay(isOpen ? null : key)} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                          <span style={{ fontFamily: DISPLAY, fontSize: 17, color: "#3b2a1a" }}>
                            {day.day}
                          </span>
                          {checked && latest?.date && (
                          ...
                          {prevCount > 0 && (
                            <span style={{ fontSize: 10, color: "#6b5639", background: "#efe3c8", borderRadius: 4, padding: "1px 6px" }}>
                    ...
                    {isOpen && (
                      <div style={{ background: "#fbf6ea", border: `1px solid ${phase.accent}30`, borderTop: "none", borderRadius: "0 0 8px 8px", padding: "4px 0 12px" }}>
                        {day.exercises.map((ex, ei) => (
                          <div key={ei} style={{ padding: "10px 16px", borderBottom: ei < day.exercises.length - 1 ? "1px solid #dccba6" : "none" }}>
                        ...
                        {day.hasTime && (
                          <div style={{ padding: "10px 16px 2px", borderTop: "1px solid #dccba6", marginTop: 4 }}>
              ...
              <div style={{ fontSize: 12, color: "#6b5639", padding: "4px 2px" }}>
                Tue · Fri · Sat · Sun — rest, or take an optional session.
              </div>
            </div>

            {/* Optional sessions */}
            <div style={{ background: "#f7efdc", border: "1px solid #cdb893", borderRadius: 8, padding: "14px 18px", marginBottom: 20 }}>
              ...
              {OPTIONAL_SESSIONS.map((s, si) => (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: si < OPTIONAL_SESSIONS.length - 1 ? "1px solid #dccba6" : "none" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, color: "#3b2a1a", fontWeight: 600 }}>{s.name}</div>
                    <div style={{ fontSize: 11, color: "#6b5639", marginTop: 2 }}>{s.detail}</div>
                  </div>
                  <button onClick={() => addEntry(s.id)} style={smallButton(phase.accent)}>+ Log today</button>
                </div>
              ))}
              {optionalThisWeek.length > 0 && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #dccba6" }}>
                  {optionalThisWeek.map(e => (
                    <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: phase.accent, padding: "3px 0" }}>
                      ...
                      <button onClick={() => removeEntry(e.id)} aria-label="Remove" style={{ ...smallButton("#6b5639"), padding: "1px 8px" }}>×</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            {activePhase === TRIAL_PHASE && (
              <div style={{ background: phase.accent + "10", border: `1px solid ${phase.accent}60`, borderRadius: 8, padding: "14px 18px", marginBottom: 20 }}>
                <div style={sectionHeading(phase.accent)}><Icon name="sword" size={18} /> The Trial · when you're ready</div>
                <div style={{ fontSize: 13, color: "#3b2a1a", fontWeight: 600 }}>{TRIAL.detail}</div>
                <div style={{ fontSize: 11, color: "#6b5639", marginTop: 4, lineHeight: 1.5 }}>{TRIAL.note}</div>
                {latestTrial && (
            ...
            {/* Milestones */}
            <div style={{ background: "#f7efdc", border: `1px solid ${phase.accent}30`, borderRadius: 8, padding: "14px 18px", marginBottom: 20 }}>
              <div style={sectionHeading(phase.accent)}>
              ...
              {phase.milestones.map((m, mi) => {
                ...
                return (
                  <div key={mi} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 0", borderBottom: mi < phase.milestones.length - 1 ? "1px solid #dccba6" : "none" }}>
                    <div style={{ marginTop: 1 }}>
                      <Checkbox checked={done} accent={phase.accent} onClick={() => toggleMilestone(m.id)} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <span style={{ fontSize: 13, color: done ? phase.accent : "#4a3826", textDecoration: done ? "line-through" : "none" }}>{m.text}</span>
                      {done && data.milestones[m.id]?.date && (
                        <span style={{ fontSize: 11, color: "#6b5639", marginLeft: 8 }}>achieved {data.milestones[m.id].date}</span>
                      )}
                    </div>
                  </div>
                );
            ...
            {phaseComplete(phase) && activePhase < phases.length - 1 && (
              <button onClick={() => { setActivePhase(activePhase + 1); setOpenDay(null); }} style={{
                width: "100%", background: phase.accent, color: "#e9dcc0", border: "none", borderRadius: 8,
                padding: "14px", fontSize: 18, cursor: "pointer",
            ...
            {phaseComplete(phase) && activePhase === phases.length - 1 && (
              <div style={{
                ...
                padding: "16px", fontSize: 19, color: phase.accent,
                fontFamily: DISPLAY, marginBottom: 20, textAlign: "center",
              }}>
                <Icon name="sword" size={20} /> The Trial is cleared. You are Umbral Assassin.
              </div>
            )}
          </>
        )}
        ...
        {activeTab === "log" && (
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: "#5f4c33", marginBottom: 16 }}>
              {logEntries.length} {logEntries.length === 1 ? "entry" : "entries"} recorded
            </div>

            {logEntries.length === 0 ? (
              <div style={{ color: "#7d6849", fontSize: 14, padding: "40px 0", textAlign: "center" }}>
              ...
              logEntries.map(e => {
                ...
                return (
                  <div key={e.uid} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, color: "#3b2a1a", fontWeight: 600, marginBottom: 2 }}>
                          {e.label}
                        </div>
                        <div style={{ fontSize: 11, color: "#6b5639" }}>
                          {e.group}
                          {e.type === "workout" && e.entry.weekLabel && (
                            <span style={{ marginLeft: 6, color: "#7d6849" }}>· week of {e.entry.weekLabel}</span>
                          )}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                        {!isEditing && (
                          <div style={{ textAlign: "right" }}>
                            ...
                            {e.type === "workout" && e.entry.time && (
                              <div style={{ fontSize: 12, color: "#5f4c33", marginTop: 1 }}>{e.entry.time}</div>
                            )}
                            {e.type === "milestone" && (
                        ...
                        <button onClick={() => setEditingEntryId(isEditing ? null : e.uid)} style={{
                          ...
                          border: `1px solid ${isEditing ? e.accent : "#bba47e"}`,
                          borderRadius: 5, padding: "4px 9px", cursor: "pointer",
                          fontSize: 11, color: isEditing ? "#e9dcc0" : "#6b5639",
                    ...
                    {!isEditing && e.type === "workout" && (e.exNotes.length > 0 || (checkable && exercises.length > 0)) && (
                      <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #dccba6" }}>
                        {checkable && exercises.length > 0 && (
                          <div style={{ fontSize: 12, color: "#5f4c33", marginBottom: 2 }}>{e.doneCount}/{exercises.length} exercises done</div>
                        )}
                        {e.exNotes.map((n, ni) => (
                          <div key={ni} style={{ fontSize: 12, color: "#6b5639", marginBottom: 2 }}>{n}</div>
                        ))}
                      </div>
                    )}

                    {isEditing && (
                      <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #dccba6", display: "flex", flexDirection: "column", gap: 10 }}>
                        ...
                        {exercises.map((ex, ei) => {
                          ...
                          return (
                            <div key={ei} style={{ paddingTop: 8, borderTop: "1px solid #dccba6" }}>
                        ...
                        {e.type === "workout" && (
                          <button onClick={() => { removeEntry(e.entry.id); setEditingEntryId(null); }}
                            style={{ ...smallButton("#9b2c2c"), alignSelf: "flex-start" }}>
                            Delete entry
                          </button>
                        )}
                      </div>
